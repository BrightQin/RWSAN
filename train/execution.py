from data.load_data import DataSet, shuffle_list
from model.model import RWSAN
from train.optim import get_optim, adjust_lr
...
class Execution:
    ...
    def train(self, dataset, dataset_eval=None):
        ...
        torch.save(self.__C, self.__C.CKPTS_PATH + 'ckpt_' + self.__C.VERSION + '/config' + '.p')
        ...
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            ...
            for step, (
                ...
                with autocast():
                    ...
                    for i in range(self.__C.IMG_FEAT_PAD_SIZE - 1, 0, -1):
                        if torch.sum(img_feat_iter[:, i, :]) != 0:
                            img_feat_iter = img_feat_iter[:, 0:i + 1, :]
                            break
                        assert i != 1

                    for i in range(self.__C.MAX_TOKEN - 1, 0, -1):
                        if torch.sum(ques_ix_iter[:, i]) != 0:
                            ques_ix_iter = ques_ix_iter[:, 0:i + 1]
                            break
                        assert i != 1
